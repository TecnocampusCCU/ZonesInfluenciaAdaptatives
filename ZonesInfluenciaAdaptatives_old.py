...
class ZonesInfluenciaAdaptatives:
    ...
    def controlErrors(self):
        ...
        try:
            ...
            errors.append('El número del radi fixe no és vàlid')
        try:
            ...
            errors.append("El percentatge de població afectada no és vàlid")
    ...
    def on_click_Recalcular(self):
        ...
        cur.execute(sql)
        conn.commit()
        drop = "drop table if exists \"EntitatPuntual_Temp\";\n"
        cur.execute(drop)
        conn.commit()
        sql="create local temp table \"EntitatPuntual_Temp\"  as select \""+ entitat + "\".*,\"Entitat_Temp\".\"NR\" from \""+ entitat + "\" join \"Entitat_Temp\" on (\""+ entitat + "\".\"id\"=\"Entitat_Temp\".\"id\");\n"
        sql+="alter table \"EntitatPuntual_Temp\" drop column \"RadiInicial\";\n"
        sql+="alter table \"EntitatPuntual_Temp\" rename column \"NR\" TO \"RadiInicial\";"
    ...
    def arxiusExisteixen(self, path):
        ...
        if self.dlg.bt_ILLES.isChecked():
            ...
            if (os.path.exists(path + "\\tr_npolicia.csv") and os.path.exists(path + "\\tr_illes.csv")):
                ...
                QMessageBox.information(None, "ERROR 2: LECTURA DELS NÚMEROS DE POLICIA", "No s'han trobat els arxius de illes ni números de policia.")
                return False
...
#       *****************************************************************************************************************
        ...
        try:
            ...
            create = "create local temp table \"TaulaPunts_Temp\" AS SELECT * FROM \"EntitatPuntual_Temp\";"
            cur.execute(create)
            conn.commit()
...
#       *****************************************************************************************************************
        ...
        try:      
            create = "create table \"JoinIlles_Habitants_Temp_"+ Fitxer + "\" AS select \"id\", \"D_S_I\", \"Habitants\", \"AV_COUNT\", \"SUM_AREA\",\"geom\" from \"Illes_Resum\", \"ILLES\" where \"ILLES_Codificades\" = \"D_S_I\";"
            cur.execute(create)
        ...
        if (self.dlg.bt_Parcel.isChecked()):
            try:
                ...
                cur.execute(drop)
                conn.commit()
                #select "Parcela","geom", "CODI_GIS", "Habitants"  from "ResumParcela", "parcel" where "Parcela" = "UTM";
                create = "create local temp table \"JoinParcel_Habitants_Temp\" AS select \"id\",\"Parcela\",\"geom\", \"CODI_GIS\", \"Habitants\" from \"Parcel_Resum\", \"parcel\" where \"Parcela\" = \"UTM\";"
                cur.execute(create)
...
#       *****************************************************************************************************************     
        try:
            ...
            conn.commit()
            if self.dlg.CB_tramsUtils.isChecked():
                sql_total="SELECT * FROM \"EntitatPuntual_Temp\""
...
#       *****************************************************************************************************************
        ...
        while i < numeroIteracions:
            ...
            if i == 0:
                ...
                if self.dlg.CB_tramsUtils.isChecked():
                    try:
                        alter = "alter table \"AgregacioSumaHab_Temp\" add column \"nouradi\" integer;\n"
                        alter += "UPDATE \"AgregacioSumaHab_Temp\" SET \"nouradi\" = round((\"radi\"*sqrt((\"NPlaces\"/\"Habitants\")*\"Cobertura\")))::integer;"
                        cur.execute(alter)
                        conn.commit()
                        drop="DROP TABLE IF EXISTS \"AgregacioNouRadi_Temp\";"
                        cur.execute(drop)
                        conn.commit()
                        sql_total="SELECT * FROM \"AgregacioSumaHab_Temp\""
                        progress.setValue(progress.value()+pas_barra_iteracions)
                        QApplication.processEvents()
                        taula_buffer=self.calcul_graf(sql_total,"nouradi")
                        create = "create local temp table \"AgregacioNouRadi_Temp\" AS select \"id\", \"radi\", \"NPlaces\", \"Habitants\", \"geom\", \"Cobertura\", \"the_geom\", \"nouradi\" from \"" + taula_buffer + "\" join \"AgregacioSumaHab_Temp\" on (\"" + taula_buffer + "\".\"punt_id\" = \"AgregacioSumaHab_Temp\".\"id\");"
                        #print create
                        cur.execute(create)
                        conn.commit()
                    except:
                    ...
                    try:
                        ...
                        cur.execute(drop)
                        conn.commit()                    
                        create = "create local temp table \"AgregacioNouRadi_Temp\" AS select \"id\", \"radi\", \"NPlaces\", \"Habitants\", \"geom\", \"Cobertura\", ST_buffer(\"geom\",round((\"radi\"*sqrt((\"NPlaces\"/\"Habitants\")*\"Cobertura\"))),100) the_geom, round((\"radi\"*sqrt((\"NPlaces\"/\"Habitants\")*\"Cobertura\"))) as NouRadi from \"AgregacioSumaHab_Temp\";"
                        #print create
                        cur.execute(create)
                        conn.commit()
                        progress.setValue(progress.value()+pas_barra_iteracions)
...
#       *****************************************************************************************************************
        try:
            drop="DROP TABLE IF EXISTS \"EntitatBase_NRS\";\n"
            cur.execute(drop)
            conn.commit()
            create = "create table \"EntitatBase_NRS\" as select *,round(\"radi\"*sqrt(\"NPlaces\"/(\"Habitants_Solapats\"*(\"habitantsreals\"/\"Suma_Habitants_Solapats\")))) NRS from \"EntitatBase\";"
            cur.execute(create)
            conn.commit()
        except:
...
#       *****************************************************************************************************************
        if self.dlg.CB_tramsUtils.isChecked():
            try:
                ...
                cur.execute(drop)
                conn.commit()
                sql_total="SELECT * FROM \"EntitatBase_NRS\""
                progress.setValue(80)
                pas_barra_iteracions=9
                QApplication.processEvents()
                taula_buffer=self.calcul_graf(sql_total,"nrs")
                create = "create local temp table \"AgregacioTotalBuffer_Temp\" AS select \"EntitatBase_NRS\".*, \"the_geom\" from \"" + taula_buffer + "\" join \"EntitatBase_NRS\" on (\"" + taula_buffer + "\".\"punt_id\" = \"EntitatBase_NRS\".\"id\");"
                #print create
                cur.execute(create)
                conn.commit()
            except:
            ...
            try:
                ...
                cur.execute(drop)
                conn.commit()
                create = "create local temp table \"AgregacioTotalBuffer_Temp\" AS select *, ST_buffer(\"geom\",\"nrs\",100) the_geom from \"EntitatBase_NRS\";"
                #print create
                cur.execute(create)
                conn.commit()
            except:
